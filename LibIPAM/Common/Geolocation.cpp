 ...
 ********************************************************************************
 */
// @todo REDO REGTESTS AS SEPARATE APP - like rest of stroika using Stroika test framework...
#if qDebug
void Geolocation::Latitude::TestSuite ()
{
    ...
    Assert (Latitude{"+10"}.ToISOString () == "+10");
    Assert (Latitude{"+1245"}.ToISOString () == "+1245");
    Assert (Latitude{"+124543"}.ToISOString () == "+124543");
    Assert (Latitude{"-2134.23"}.ToISOString () == "-2134.23");
    Assert (Latitude{"+124543.891234"}.ToISOString () == "+124543.891234");
}
...
void Geolocation::Longitude::TestSuite ()
{
    ...
    Longitude d{"+21234.23"};
    Assert (d.degrees () == 212);
    Assert (d.minutes () == 34);
    Assert ((abs (d.seconds ()) - .23) < 0.00001);

    Longitude e{"-21234.23"};
    Assert (e.degrees () == -212);
    Assert (e.minutes () == -34);
    Assert ((abs (e.seconds ()) - .23) < 0.00001);

    Assert (Longitude{"+100"}.ToISOString () == "+100");
    Assert (Longitude{"+12345"}.ToISOString () == "+12345");
    Assert (Longitude{"+1234543"}.ToISOString () == "+1234543");
    Assert (Longitude{"-21234.23"}.ToISOString () == "-21234.23");
    Assert (Longitude{"+1234543.891234"}.ToISOString () == "+1234543.891234");
}
#endif
...
void Geolocation::TestSuite ()
{
    ...
    Geolocation a ("+1245", "+100");
    Assert (a.latitude == Latitude{"+1245"});
    Assert (a.longitude == Longitude{"+100"});
    Assert (a.ToISOString () = "+1245+100/");

    Assert (Geolocation ("-2134.23", "+1234543.891234").ToISOString () = "-2134.23+1234543.891234/");

    // examples from https://en.wikipedia.org/wiki/ISO_6709
    Assert (Geolocation{"+00-025/"}.ToISOString () = "+00-025/");                                                 // Atlantic Ocean
    Assert (Geolocation{"+48.8577+002.295/"}.ToISOString () = "+48.8577+002.295/");                               // Eiffel Tower
    Assert (Geolocation{"+27.5916+086.5640+8850CRSWGS_84/"}.ToISOString () = "+27.5916+086.5640+8850CRSWGS_84/"); // Mount Everest
    Assert (Geolocation{"-90+000+2800CRSWGS_84/"}.ToISOString () = "-90+000+2800CRSWGS_84/");                     // South Pole
    Assert (Geolocation{"+40.6894-074.0447/"}.ToISOString () = "+40.6894-074.0447/");                             // Statue of Liberty
}
#endif
